export default function FileItem({ file, onClick, onContextMenu, formatSize, formatDate, isTrashView = false }) {
  ...
  return (
    <div
      ...
      className="flex items-center grid-cols-12 gap-4 px-6 py-3 hover:bg-canvas-soft transition-colors cursor-pointer group"
    >
      <div className="col-span-5 flex items-center">
        <div className="mr-3">
          {file.is_folder ? folderIcon : getFileIcon(file.name)}
        </div>
        <span className="text-body-md text-ink group-hover:text-link transition-colors truncate">
          {file.name}
        </span>
        {file.is_folder && (
          <svg className="w-4 h-4 ml-2 text-mute" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          </svg>
        )}
      </div>
      <div className="col-span-2 text-right text-body-sm text-body">
